...
class ClassesProvider extends ChangeNotifier {
  ...
  /// Сырой ответ последнего УСПЕШНОГО `GET /classes/` — его переиспользует лента
  /// уведомлений. `null` — членство ни разу не грузилось, фид сходит сам.
  List<dynamic>? _myClassesRaw;

  /// Загрузка членства «в полёте»: не пускает второй такой же запрос и даёт
  /// [loadNotifBadge] точку синхронизации вместо своей копии запроса.
  ...
  Future<void> loadNotifBadge() async {
    ...
    try {
      // Список классов берём у загрузки членства, а не запрашиваем свой: если
      // она в полёте — ждём её, если отработала — берём готовый, и только если
      // не грузилось ни разу — запускаем сами.
